	...
	*/
	...
	for (size_t idx = 0; idx < sizeOrthoGroupTables; idx++) //Loop over every table
	{
		...
		for (size_t idx2 = 0; idx2 < sizeOrthoGroupRows; idx2++) //Loop over every row
		{
			...
			strFastaFileNameOrthoGroup = pvecOrthoGroupRow->at(0); //Set the Namehttps://docs.gwdg.de/lib/exe/fetch.php?media=en:services:network_services:eduroam:macos_eduroam-install_eng_00.png
