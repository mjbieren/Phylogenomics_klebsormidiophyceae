...
long WriteSummaryFile(size_t sizetotalOrthoGroups, size_t sizeFilteredOrthoGroups, char ** ppFileBuffer, size_t sizeBuffer, size_t sizeFastaFilesSource)
{
	...
	*/
	...
	sizeBufferLeft = sizeBufferLeft - (pBufferPos - *ppFileBuffer);
	pBufferPos += sprintf_s(pBufferPos, sizeBufferLeft, "From the total off %zu Orthogroups, %zu Where Filtered out resulting in %zu Orthogroups, containing at least %zu Taxonomic Groups.%s", sizetotalOrthoGroups, sizetotalOrthoGroups-sizeFilteredOrthoGroups, sizeFilteredOrthoGroups, sizeThreshold,EOL); //LINE FOUR
	sizeBufferLeft = sizeBufferLeft - (pBufferPos - *ppFileBuffer);
